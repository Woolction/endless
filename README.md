...
2. a recommendation system based on preference vectors.
3. authentication and authorization (creating a user, channel, etc.)
4. user interactions (comments, likes, dislikes, saving videos, etc.)
...
- Open API whitc Scalar

## Tech Stack
`C#` `ASP.NET Core` `PostgreSQL` `Entity Framework core` `Dapper` `Elasticsearch` `Docker compose` 
